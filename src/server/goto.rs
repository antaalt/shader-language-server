...
impl ServerLanguage {
    pub fn recolt_goto(
        ...
        match self.get_symbol_provider(cached_file.shading_language).get_word_range_at_position(&cached_file.content, &file_path, shader_position.clone()) {
                        ...
                        .filter_map(|symbol| if let ShaderSymbolData::Link { target } = &symbol.data {
                            match &symbol.range {
                                ...
                                Some(_range) => Some(lsp_types::LocationLink {
                                    origin_selection_range: Some(shader_range_to_lsp_range(&word_range)),
